...
export function registerLayoutTools(
  ...
  server.registerTool('sidebar_get_sessions', {
    description: 'List sessions shown in the sidebar with their status (excludes terminal sessions)',
    annotations: { readOnlyHint: true },
  }, async () => {
    try {
  ...
  server.registerTool('sidebar_switch_tab', {
    description: 'Switch the sidebar to a specific tab (sessions, search, changes, stats, activity)',
    annotations: { readOnlyHint: false },
    inputSchema: {
      tab: z.enum(['sessions', 'search', 'changes', 'stats', 'activity']).describe('The sidebar tab to switch to'),
    },
  }, async ({ tab }) => {
    try {
