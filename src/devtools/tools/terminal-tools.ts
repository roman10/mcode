...
export function registerTerminalTools(
  ...
  server.tool(
    ...
    'Read text content from the terminal buffer',
    {
  ...
  server.tool(
    'terminal_send_keys',
    'Send keystrokes or text to the terminal. Use \\r for Enter, \\x03 for Ctrl+C, etc.',
    {
      sessionId: z.string().describe('The PTY session ID'),
      keys: z
        .string()
  ...
  server.tool(
    ...
    'Get the current terminal dimensions (columns and rows)',
    {
      sessionId: z.string().describe('The PTY session ID'),
    },
    async ({ sessionId }) => {
  ...
  server.tool(
    'terminal_wait_for_content',
    'Wait until a regex pattern appears in the terminal buffer. Polls every 250ms.',
    {
      sessionId: z.string().describe('The PTY session ID'),
      pattern: z
        .string()
