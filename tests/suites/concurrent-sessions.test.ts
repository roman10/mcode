...
describe('concurrent sessions', () => {
  ...
  it('all sessions have tiles auto-added', async () => {
    // Give time for auto-tile IPC events
  ...
  it('tile count returns to baseline after kills', async () => {
    // Give renderer time to process all exits
    await new Promise((r) => setTimeout(r, 500));

    // After killing all test sessions, tiles may or may not auto-remove.
    // The key assertion: tile count should be a non-negative number.
